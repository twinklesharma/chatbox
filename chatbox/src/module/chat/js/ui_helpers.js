...
function buildMessageHTML(messageText, messageSenderId, messageDateSent, attachmentFileId, messageId, status){
  var messageAttach;
  if(attachmentFileId){
      messageAttach = '<a href="' + QB.content.publicUrl(attachmentFileId) + '/' + '/download.xml?token='+token+'" target="_new"><img src="' + QB.content.publicUrl(attachmentFileId) + '/' + '/download.xml?token='+token+'" alt="" class="attachments img-responsive" height="50px" width="50px" onerror=this.src="https://img.clipartfest.com/9f9621cc6768852c9386988a8fbbba59_empty-unix-file-clip-art-text-file-clipart_407-470.svg" /></a>';
  }

	var isMessageSticker = stickerpipe.isSticker(messageText);

  var delivered = '<img class="icon-small" src="laravelchat/images/delivered.jpg" alt="" id="delivered_'+messageId+'">';
  var read = '<img class="icon-small" src="laravelchat/images/read.jpg" alt="" id="read_'+messageId+'">';
